...
info(cpu_topology) ->
  ?IO('Maybe':'maybe'(system_info(cpu_topology)));
...
info(modified_timing_level) ->
  ?IO('Maybe':'maybe'(system_info(modified_timing_level)));
...
stats(scheduler_wall_time) ->
  ?IO('Maybe':'maybe'(statistics(scheduler_wall_time)));
stats(scheduler_wall_time_all) ->
  ?IO('Maybe':'maybe'(statistics(scheduler_wall_time_all)));
